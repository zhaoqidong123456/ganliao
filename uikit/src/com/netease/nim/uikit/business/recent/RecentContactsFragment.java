...
public class RecentContactsFragment extends TFragment {
    ...
    private void findViews() {
        ...
        TextView allRead = findView(R.id.all_read);
        allRead.setOnClickListener(v -> {
            NIMClient.getService(MsgService.class).clearAllUnreadCount();
        });
        findView(R.id.testaaaa).setOnClickListener(v -> {
            UIKitManager.getInstance().callVideoListener.testExportMsg(getActivity());
        });
        RecentContactsItem newsRecord = findView(R.id.news_record);
        newsRecord.setOnClickListener(v -> {
            RecentContactsHeardListener heardListener = UIKitManager.getInstance().heardListener;
            if (heardListener==null){
                return;
            }
            heardListener.onRecentContactsRecordListener(getActivity());
        });
        RecentContactsItem newsNews = findView(R.id.news_news);
        newsNews.setOnClickListener(v ->{
            RecentContactsHeardListener heardListener = UIKitManager.getInstance().heardListener;
            if (heardListener==null){
                return;
            }
            heardListener.onRecentContactsNewsListener(getActivity());
        });
        RecentContactsItem newsKefu = findView(R.id.news_kefu);
        newsKefu.setOnClickListener(v -> {
            RecentContactsHeardListener heardListener = UIKitManager.getInstance().heardListener;
            if (heardListener==null){
                return;
            }
            heardListener.onRecentContactsKefuListener(getActivity());
        });
    }
    /**
     * 初始化消息列表
...
//            showLongClickMenu(adapter.getItem(position), position);
    ...
    private void onRecentContactsLoaded() {
        ...
        View headView = getLayoutInflater().inflate(R.layout.item_chat_recent_default_item, null);
        //添加头布局尾布局
        adapter.addHeaderView(headView, 1);
    }
            ...
            if (recentContact != null) {
                for (RecentContact item : items) {
                    if (TextUtils.equals(item.getContactId(), recentContact.getContactId()) &&
                        ...
                        if (item.getContactId().contains("uans90321609143164")){
                            items.remove(item);
                        }
                        items.remove(item);
                        refreshMessages(true);
