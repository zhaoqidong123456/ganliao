...
public
class Endpresenter implements IEndpresenter{
    private Service service;
    private Endview endview;

    public Endpresenter(Endview endview) {
        this.endview = endview;
        service= RetrofitUtils.getRetrofitUtils().getService();
    }

    @Override
    public void loadData(String channed_id) {
        final String string= RetrofitUrl.key+RetrofitUrl.channel+channed_id+RetrofitUrl.version1;
        String string1 = MD5Utils.MD5(string);
        String string2 = string1.toUpperCase();
        HashMap<String, String> stringStringHashMap = new HashMap<>();
        stringStringHashMap.put("channel", RetrofitUrl.channel);
        stringStringHashMap.put("channel_id",channed_id);
        stringStringHashMap.put("sig",string2);
        stringStringHashMap.put("version",RetrofitUrl.version1);
        service.tong(stringStringHashMap)
                ...
                .subscribe(new Observer<ResponseBody>() {
                    ...
                    public void onNext(@NonNull ResponseBody responseBody) {
                        try {
                            ...
                            Log.e("tonghua",string3);
                            Gson gson = new Gson();
                            if (string3.contains("duration")){
                                Endbean endbean = gson.fromJson(string3, Endbean.class);
                                endview.showData(endbean);
                            }else {
