...
public
class Paopresenter implements IPaopresenter{
    private Service service;
    private Paoview paoview;

    public Paopresenter(Paoview paoview) {
        this.paoview = paoview;
        service= RetrofitUtils.getRetrofitUtils().getService();
    }

    @Override
    public void loadData(String type) {
        final String string= RetrofitUrl.key+RetrofitUrl.channel+type+RetrofitUrl.version1;
        ...
        service.paomadeng(stringStringHashMap)
                    ...
                    public void onNext(@NonNull ResponseBody responseBody) {
                        try {
                            ...
                            if (string3.contains("nickname")){
                                Paobean paobean = gson.fromJson(string3, Paobean.class);
                                paoview.showData(paobean);
                            }else {
    ...
    @Override
    public void loadDatavip(String type) {
        final String string= RetrofitUrl.key+RetrofitUrl.channel+type+RetrofitUrl.version1;
        String string1 = MD5Utils.MD5(string);
        String string2 = string1.toUpperCase();
        HashMap<String, String> stringStringHashMap = new HashMap<>();
                ...
                .subscribe(new Observer<ResponseBody>() {
                    ...
                    public void onNext(@NonNull ResponseBody responseBody) {
                        try {
                            ...
                            Gson gson = new Gson();
                            if (string3.contains("nickname")){
                                Paobeanvip paobean = gson.fromJson(string3, Paobeanvip.class);
                                paoview.showDatavip(paobean);
                            }else {
