...
public class Attenpresenter implements IAttenpresenter{
    private Service service;
    private Attenview attenview;

    public Attenpresenter(Attenview attenview) {
        this.attenview = attenview;
        service= RetrofitUtils.getRetrofitUtils().getService();
    }

    @Override
    public void loadData(String userid, String add_id, String type) {
        final String string= RetrofitUrl.key+RetrofitUrl.channel+add_id+type+userid+RetrofitUrl.version1;
                ...
                .subscribe(new Observer<ResponseBody>() {
                    ...
                    public void onNext(ResponseBody responseBody) {
                        try {
                            ...
                            //Log.e("guanzhu",string3);
                            Gson gson = new Gson();
                            if (string3.contains("1")){
                                Attenbean attenbean = gson.fromJson(string3, Attenbean.class);
                                attenview.showatt(attenbean);
                            }else {
                                Yzmfbean yzmfbean = gson.fromJson(string3, Yzmfbean.class);
                                attenview.showattf(yzmfbean);
                            }
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
