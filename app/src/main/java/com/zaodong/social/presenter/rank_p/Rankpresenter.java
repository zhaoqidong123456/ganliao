package com.zaodong.social.presenter.rank_p;
...
public class Rankpresenter implements IRankpresenter{
    ...
    @Override
    public void loadData(String userid, String type, String subtype) {
        final String string= RetrofitUrl.key+RetrofitUrl.channel+subtype+type+userid+RetrofitUrl.version1;
        ...
        stringStringHashMap.put("subtype",subtype);
        stringStringHashMap.put("sig",string2);
        stringStringHashMap.put("version",RetrofitUrl.version1);
        Log.e("rank",type+"   "+subtype+"");
        service.rank(stringStringHashMap).subscribeOn(Schedulers.newThread())
                    ...
                    public void onNext(ResponseBody responseBody) {
                        try {
                            ...
                            if (string3.contains("1")){
                                Rankbean rankbean = gson.fromJson(string3, Rankbean.class);
                                rankview.showRank(rankbean);
                            }else {
                                Yzmfbean yzmfbean = gson.fromJson(string3, Yzmfbean.class);
                                rankview.showRankf(yzmfbean);
                            }
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
