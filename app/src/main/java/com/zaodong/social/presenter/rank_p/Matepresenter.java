...
public class Matepresenter implements IMatepresenter{
    private Service service;
    private Mateview mateview;

    public Matepresenter(Mateview mateview) {
        this.mateview = mateview;
        service= RetrofitUtils.getRetrofitUtils().getService();
    }

    @Override
    public void loadmate(String userid, String page, String counts) {
        ...
        service.intimatelist(stringStringHashMap)
                    ...
                    public void onNext(ResponseBody responseBody) {
                        try {
                            ...
                            if (string3.contains("1")){
                                Matebean matebean = gson.fromJson(string3, Matebean.class);
                                mateview.showDatamate(matebean);
                            }else {
                                Yzmfbean yzmfbean = gson.fromJson(string3, Yzmfbean.class);
                                mateview.showDatamatef(yzmfbean);
                            }
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
